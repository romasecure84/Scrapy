...
class RotateUserAgentAndProxyMiddleware:
    ...
    def process_request(self, request, spider):
        ...
        spider.logger.info(f'Current User-Agent: {user_agent}, Current Proxy: {proxy}')

    def process_response(self, request, response, spider):
        if response.status != 200:
            user_agent = random.choice(self.user_agents)
            proxy = random.choice(self.proxies)
            spider.logger.info(f'Request unsuccessful, New User-Agent: {user_agent}, Current Proxy: {proxy}')
    ...
    def process_exception(self, request, exception, spider):
        user_agent = random.choice(self.user_agents)
        proxy = random.choice(self.proxies)
        spider.logger.info(f'Exception: {exception}, New User-Agent: {user_agent}, Current Proxy: {proxy}')
        new_request = request.copy()
        new_request.headers['User-Agent'] = user_agent
        new_request.meta['proxy'] = proxy
        new_request.dont_filter = True
...
class RotateUserAgentMiddleware:
    ...
    def process_request(self, request, spider):
        user_agent = random.choice(self.user_agents)
        request.headers['User-Agent'] = user_agent
        spider.logger.info(f'Current User-Agent: {user_agent}')
